...
export default function ProductImagesUploader({
    ...
    try {
      ...
      setUrls((prev) =>
        result.galleryUrls && result.galleryUrls.length > 0
          ? result.galleryUrls
          : [...prev, result.publicUrl as string],
      );
  ...
  async function handlePick(event: ChangeEvent<HTMLInputElement>) {
    ...
    // Upload in parallel — Supabase signed URLs handle concurrency fine.
  ...
  function removeAt(url: string) {
    ...
    startTransition(async () => {
      const optimistic = urls.filter((u) => u !== url);
      setUrls(optimistic);
  ...
  const hasAny = urls.length > 0 || pending.length > 0;
  const cover = urls[0];
    ...
    <div>
      <div className="flex items-baseline justify-between mb-2">
        ...
        <span className="text-[11px] tabular-nums text-[#868685]">
          {urls.length + pending.length}/{MAX_GALLERY}
        </span>
      </div>

      {/* Cover card — primary image, larger */}
      {cover && (
        <div className="mb-3">
          ...
          <div className="mt-2 flex items-center gap-2">
            <span className="text-[11px] font-bold uppercase tracking-[0.16em] px-2 py-1 rounded-full bg-[#0e0f0c] text-white">
              Cover
            </span>
            <button
              type="button"
              onClick={() => removeAt(cover)}
              className="text-[12px] font-semibold text-[#454745] hover:text-[#a3221a] h-8 px-3 rounded-full hover:bg-[#fde2df] transition-colors"
            >
              Remove
            </button>
          </div>
      ...
      {remainingSlots > 0 && (
        <div className="rounded-2xl border-[1.5px] border-dashed border-[rgba(14,15,12,0.14)] bg-white px-5 py-6 flex items-center justify-between gap-4 flex-wrap">
          <div className="text-[13px] text-[#454745] leading-[1.5]">
            {hasAny
              ? `Add up to ${remainingSlots} more. JPEG, PNG, WebP, GIF, AVIF — 5 MB each.`
              : "JPEG, PNG, WebP, GIF, or AVIF. Up to 5 MB each, 8 total. The first image becomes the cover."}
          </div>
      ...
      {bulkError && (
        <p className="mt-2 text-[13px] font-medium text-[#da291c]">
          {bulkError}
        </p>
      )}
    </div>
